...
def is_distance_valid(row):
    ...
    distance = consts.EARTH_MEAN_RADIUS * math.acos(
        math.sin(lat) * math.sin(lat_geo) + math.cos(lat) * math.cos(lat_geo) * math.cos(long - long_geo))
...
def reverse_geocoding(row):
    ...
    with urllib.request.urlopen(url) as response:
        ...
        for object in json_response:
            try:
                if row[consts.LOCATION] != consts.YOSHKAR_OLA and \
                        object['GeoObject']['metaDataProperty']['GeocoderMetaData']['AddressDetails']['Country'][
                            'AdministrativeArea']['AdministrativeAreaName'] == consts.MARI_EL:
                    return True
            except KeyError:
                pass
...
def check_confused_coordinates(row):
    try:
        ...
        if (consts.MARI_EL_EAST > latitude > consts.MARI_EL_WEST and
                consts.MARI_EL_NORTH > longitude > consts.MARI_EL_SOUTH):
...
with open('../data/input_geocoded.csv', encoding='utf-8', mode='r') as input, open('../data/input_verified.csv',
    ...
    for row in input:
        ...
        # verify coordinates
        if row[consts.LOCATION] == consts.YOSHKAR_OLA and (
                longitude > consts.YOSHKAR_OLA_EAST or longitude < consts.YOSHKAR_OLA_WEST or
                latitude > consts.YOSHKAR_OLA_NORTH or latitude < consts.YOSHKAR_OLA_SOUTH):
            row.append(0)
        elif (longitude > consts.MARI_EL_EAST or longitude < consts.MARI_EL_WEST or
              latitude > consts.MARI_EL_NORTH or latitude < consts.MARI_EL_SOUTH):
            row.append(0)
        else:
