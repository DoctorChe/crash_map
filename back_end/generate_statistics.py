...
def print_location_statistics(index):
    ...
    print('ДТП с более чем одним пострадавшим:', accidents_injury_multiple[index],
          get_percentege(accidents_injury_multiple[index], accidents_injury[index]))
    ...
    print('ДТП с более чем одним погибшим:', accidents_death_multiple[index],
          get_percentege(accidents_death_multiple[index], accidents_death[index]))
    print('Погибло в ДТП:', death[index], '\n')

    print('ДТП с пострадавшими детьми:', accidents_injury_children[index],
          get_percentege(accidents_injury_children[index], accidents[index]))
    print('ДТП с более чем одним пострадавшим ребёнком:', accidents_injury_multiple_children[index],
          get_percentege(accidents_injury_multiple_children[index], accidents_injury_children[index]))
    print('Пострадало детей в ДТП:', injury_children[index], '\n')

    print('ДТП с погибшими детьми:', accidents_death_children[index],
          get_percentege(accidents_death_children[index], accidents[index]))
    ...
    print('Всего ДТП с адресами:', accidents_with_address[index],
          get_percentege(accidents_with_address[index], accidents[index]))
    print('Всего ДТП с адресами, которые удалось геокодировать:', accidents_with_address_geocoded[index],
          get_percentege(accidents_with_address_geocoded[index], accidents_with_address[index]), '\n')

    print('Координат, не попадающих на территорию:', incorrect_coordinates[index],
          get_percentege(incorrect_coordinates[index], accidents[index]))
    print('Координат, не попадающих в окружность радиусом 150 метров вокруг указанного в данных адреса:',
          incorrect_coordinates_strict[index],
          get_percentege(incorrect_coordinates_strict[index], accidents_with_address[index]), '\n')


def print_day_of_week_stat(index):
    for day in range(7):
        print(day_of_week[day] + ':', day_of_week_stat[index][day],
              get_percentege(day_of_week_stat[index][day], accidents[index]))


def print_month_valid_stat(index):
    for month_index in range(12):
        print(month[month_index] + ':',
              round(month_valid_stat[index][month_index] * 100 / month_stat[index][month_index], 2))
...
print('\nКоличество ДТП типа "Падение пассажира" без пострадавших:', easy_fall,
      get_percentege(easy_fall, type_map['Падение пассажира'][0]), '\n')
